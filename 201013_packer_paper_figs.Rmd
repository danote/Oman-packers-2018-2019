...
# Sulfate
```{r}
data %>% filter(sampling_site == "WAB55") %>% select(sampling_site, year_sampled, Sulfate_uM)
```

```{r}
data %>% filter(Sulfate_uM>2000) %>% select(sampling_site, year_sampled, Sulfate_uM)
```
...
sulfate_plot <- ggplot() +
    ...
    # points
    geom_jitter(data = data %>% filter(packer_site_year_interval == "Other Samail Ophiolite"),
    mapping =  aes(
    x = water_type ,
    y = Sulfate_uM,
    # shape = packer_site_year_interval,
    fill = packer_site_year_interval),
    width = 0.1, shape = 21, alpha = 0.4) +
  ...
  geom_jitter(data = data %>% filter(packer_site_year_interval != "Other Samail Ophiolite"),
    ...
    fill = packer_site_year_interval),
    width = 0.1, shape = 21) +
  scale_fill_manual(values = colors_all_packer_site_year_interval_named) +
  theme_bw()
