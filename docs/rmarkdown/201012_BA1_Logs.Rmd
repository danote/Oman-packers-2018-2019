...
BA1_profiles <- full_join((BA1A_2018_profile %>% mutate(sampling_site = "BA1A", year_sampled = 2018, group_id = "BA1A_2018_log") %>% filter(depth_sampled_mbgl > 22)), (BA1A_2017_profile %>% mutate(sampling_site = "BA1A", year_sampled = 2017, group_id = "BA1A_2017_log") %>% filter(depth_sampled_mbgl > 22))) %>% full_join((BA1D_2018_profile  %>% mutate(sampling_site = "BA1D", year_sampled = 2018, group_id = "BA1D_2018_log") %>% filter(depth_sampled_mbgl > 26))) %>% arrange(depth_sampled_mbgl)
...
BA1_profiles_and_packer_geochem <- full_join((BA1_profiles_longer %>% mutate(sample_method = "Wireline log")), (Oman_Geochem_BA1_longer_depth %>% mutate(sample_method = "Pumped sample")))
```
  ...
  mutate(
    tex_params = as_factor(parameter) %>%
      fct_recode(
        ...
        "Conductivity$\\,$/$\\,$$\\lbrack$mS$\\cdot$cm$^{-1}$$\\rbrack$" = "cond_mS_per_cm"
  )
  )
```

## Plot
```{r BA1_profiles_plot, fig.width=7, fig.height=5}
BA1_profiles_plot <- BA1_profiles_and_packer_geochem %>% ggplot(aes(
  ...
  # plot styling
  scale_color_manual(name = "Year sampled", values = c("2019" = "#E69F00", "2018" = "black", "2017" = "#0072B2"))+
  scale_size_manual(name = "Sampling method", values = c("Wireline log" = 0.2, "Pumped sample" = 2.5)) +
  ...
  theme(
    ...
    strip.background.y = element_rect(color = "black", fill = "grey80"),
