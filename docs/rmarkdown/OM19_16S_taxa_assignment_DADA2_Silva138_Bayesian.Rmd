...
library(ShortRead);
packageVersion("ShortRead") # determining primer reads
library(Biostrings);
packageVersion("Biostrings") # determining primer reads
...
Define where the fastq file are and check what the files are. These fastq files were previously demultiplexed.
```{r set-fastq-files-directory}
path <- "~/Desktop/Research/Oman_fieldwork/2019 info/Dan_Nothaft_16S_Data/demux-seqs/packers" # CHANGE ME to the directory containing the fastq files after unzipping.
list.files(path)
```
...
sample.names <- sapply(strsplit(basename(fnFs), "_S[[:digit:]]_L001"), `[`, 1)
...
The forward reads are good quality. DADA2 generally advises trimming the last few nucleotides to avoid less well-controlled errors that can arise there. We will truncate the forward reads at position 260.
...
The reverse reads also look to be of generally good quality, but the quality decreases more with cycle number than the forward reads. This is common.  Thus, we will truncate the reverse reads at position 200, which is a bit shorter than the forward read truncation.

The chosen truncation lengths seem pretty conservative. Through several iterations of truncation lengths, it was determined that keeping more reads, particularly for reverse reads, led to ~50% loss of reads at the DADA2 chimera filtering step. With the truncation lengths selected here, ~85% of reads are retained at this chimera filtering step, presumably due to higher overall read quality achieve by truncating lower quality reads in the reverse cycle.
...
We lose a considerable amount of reads (~20%) due to trimming, but that's OK.
```{r filter-and-trim}
# filtering out primer length using trimLeft argument
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(260, 200), trimLeft = c((str_length(FWD)), (str_length(REV))),
...
As a sanity check, we will count the presence of primers in the first filtered sample:
...
Success! The vast majority of primer sequences have been removed. 
...
dadaFs[[1]]
```
The DADA2 algorithm inferred ~300 true sequence variants from the ~13000 unique sequences in the first sample.
...
```
This table shows the number of sequences in the dataset of various lengths. Here, you want to check that the lengths of our merged sequences all fall within the expected range for your amplicon. The sequences in this dataset are 252 bp length, which seems reasonable.
...
This double checks that you didn't just toss most of your reads, if this percentage is low it means that few of your total reads remain and then you need to go back. In this dataset, chimeras account for about 15% of the merged sequence reads, which is decent.
...
write(asv_fasta, file = format(Sys.Date(), "data_output/%Y%m%d_ASVs_Silva_OM19_Silva138_Bayes_packers.fa"))

  # count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, file = format(Sys.Date(), "data_output/%Y%m%d_ASVs_counts_OM19_Silva138_Bayes_packers.txt"), sep="\t", quote=F)

  # tax table:
asv_tax <- taxa_Silva
row.names(asv_tax) <- sub(">", "", asv_headers)
write.table(asv_tax, file = format(Sys.Date(), "data_output/%Y%m%d_ASVs_taxonomy_OM19_Silva138_Bayes_packers.txt"), sep="\t", quote=F)
```
...
write.table(tax_tab_for_mctoolsr_Silva, file = format(Sys.Date(), "data_output/%Y%m%d_tax_tab_DADA2_mctoolsr_OM19_Silva138_Bayes_packers.txt"), row.names = FALSE, sep="\t", quote=F) # write a .txt file of the taxa table
```