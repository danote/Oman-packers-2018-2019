...
path <- "~/Desktop/Research/Oman_fieldwork/2019 info/Dan_Nothaft_16S_Data/demux-seqs" # CHANGE ME to the directory containing the fastq files after unzipping.
list.files(path)
```
...
write(asv_fasta, file = format(Sys.Date(), "data_output/%Y%m%d_ASVs_Silva_Bayes.fa"))

  # count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, file = format(Sys.Date(), "data_output/%Y%m%d_ASVs_counts_Silva_Bayes.txt"), sep="\t", quote=F)

  # tax table:
asv_tax <- taxa_Silva
row.names(asv_tax) <- sub(">", "", asv_headers)
write.table(asv_tax, file = format(Sys.Date(), "data_output/%Y%m%d_ASVs_taxonomy_Silva_Bayes.txt"), sep="\t", quote=F)
```
...
write.table(tax_tab_for_mctoolsr_Silva, file = format(Sys.Date(), "data_output/%Y%m%d_tax_tab_DADA2_mctoolsr_OM19_Silva138_Bayes_full.txt"), row.names = FALSE, sep="\t", quote=F) # write a .txt file of the taxa table
```