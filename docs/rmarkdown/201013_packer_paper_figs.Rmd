...
## Plot
```{r Si-pH-plot-BA1, fig.width=10, fig.height=5, warning=FALSE}
# create plot
Si_pH_plot <- ggplot() +
...
# context data Leong, canovas, stanger, chavagnac
  ...
  geom_point(data = data %>% filter(water_type == "Ca_OH" | water_type == "Mg_HCO3") %>% filter(str_detect(packer_site_year_interval, "BA1") == FALSE),
    ...
    y = Si_total_uM,
    shape = packer_site_year_interval,
    color = packer_site_year_interval), size = 5.5) +
  ...
  # annotations
  ggplot2::annotate("text", label = "Rain", x = 5.8,  y = 0.4, color = "black", size = 5) +
  ggplot2::annotate("text", label = TeX("Mg$^{2+}$ - HCO$_{3}^{-}$"), x = 7.6,  y = 700, angle = 0, color = "black", parse = TRUE, size = 5) +
  ggplot2::annotate("text", label = "Intermediate", x = 8.9,  y = 0.3, angle = -63, color = "black", size = 5) +
  ggplot2::annotate("text", label = TeX("Ca$^{2+}$ - OH$^{-}$"), x = 11.8,  y = 0.09, angle = 50, color = "black", parse = TRUE, size = 5) +
  ggplot2::annotate("text", label = "Mix", x = 11,  y = 400, angle = 0, color = "black", size = 5) +

    ggplot2::annotate("text", label = "1%", x = 12.7,  y = data_Leong_mix_all_endmembers_01_percent_mix %>% summarise(Si_umolal = max(Si_molal) * 1e6) %>% as.numeric(), angle = 0, color = "blue", size = 5) +
    ggplot2::annotate("text", label = "5%", x = 12.7,  y = data_Leong_mix_all_endmembers_05_percent_mix %>% summarise(Si_umolal = min(Si_molal) * 1e6) %>% as.numeric(), angle = 0, color = "blue", size = 5) +
    ggplot2::annotate("text", label = "10%", x = 12.7,  y = data_Leong_mix_all_endmembers_10_percent_mix %>% summarise(Si_umolal = max(Si_molal) * 1e6) %>% as.numeric(), angle = 0, color = "blue", size = 5) +
    ggplot2::annotate("text", label = "50%", x = 12.45,  y = 170, angle = 0, color = "blue", size = 5) +
  ggplot2::annotate("text", label = "90%", x = 10,  y = 450, angle = 0, color = "blue", size = 5) +
