...
```{r load-data-file-paths}
tax_table_fp <- 'data_output/tax_tab_DADA2_mctoolsr_OM18_Silva138_Bayes_packers.txt'
map_fp <- 'data_raw/Summary_metadata/meta_map_16S_OM18_packers.txt'
```
...
Oman_samples <- input_filt %>% filter_data(filter_cat = "sample_ID", keep_vals = c( "BA1A5566_45_1J", "BA1A100_10_2J", "BA1A100_45_2C", "BA1A5566_22_1C","BA1A100_22_2D", "BA1A5566_10_1D"))
```
...
```{r heat-map-OM18-packers-all-filters, fig.height=7.5, fig.width=10.2}
Oman_samples_sum_higher_plot <- plot_ts_heatmap(Oman_samples_sum, map_Oman_samples, 0.005,  "sample_ID")
...
```{r input-Silanimonas-taxa}
# input methane cycle
Xanthomonadaceae <- c("Xanthomonadaceae")
```

### filter for all Silanimonas taxa
```{r filter-for-Silanimonas-taxa, warning=FALSE}
# filter for CH4-cycling taxa
Oman_samples_sum_all_Xanthomonadaceae <- filter_taxa_from_table(tax_table =  Oman_samples_sum, taxa_to_keep = Xanthomonadaceae)

# get rownames of CH4 cycle taxa data frame
tax_long <- tibble(rownames(Oman_samples_sum_all_Xanthomonadaceae))
colnames(tax_long) <- "tax_long"

# Reshaping data so it is easier to work with

# make rownames into a column
Oman_samples_sum_all_Xanthomonadaceae <- bind_cols(tax_long, Oman_samples_sum_all_Xanthomonadaceae)

# print
Oman_samples_sum_all_Xanthomonadaceae
```

### show full taxonomy of Xanthomonadaceae taxa in easily searchable format
```{r show-Xanthomonadaceae-searchable}
# gather data
Oman_samples_sum_all_Xanthomonadaceae_long_gathered <- Oman_samples_sum_all_Xanthomonadaceae %>%  gather(key = "sample_ID", value = "rel_abund", -tax_long)

# pretty print
Oman_samples_sum_all_Xanthomonadaceae_long_gathered %>% kable()
```

Add a shorthand name for taxa
```{r add-Xanthomonadaceae-short}
Oman_samples_sum_all_Xanthomonadaceae <- Oman_samples_sum_all_Xanthomonadaceae  %>% mutate(tax_short = case_when(
  tax_long == "k__Bacteria; p__Proteobacteria; c__Gammaproteobacteria; o__Xanthomonadales; f__Xanthomonadaceae; g__Lysobacter; s__NA" ~ "genus Lysobacter",
  tax_long == "k__Bacteria; p__Proteobacteria; c__Gammaproteobacteria; o__Xanthomonadales; f__Xanthomonadaceae; g__NA; s__NA" ~ "genus unassigned",
  tax_long == "k__Bacteria; p__Proteobacteria; c__Gammaproteobacteria; o__Xanthomonadales; f__Xanthomonadaceae; g__Pseudoxanthomonas; s__NA" ~ "genus Pseudoxanthomonas",
  tax_long == "k__Bacteria; p__Proteobacteria; c__Gammaproteobacteria; o__Xanthomonadales; f__Xanthomonadaceae; g__Silanimonas; s__NA" ~ "genus Silanimonas"
))

# put into easily searchable format
Oman_samples_sum_all_Xanthomonadaceae_gathered <- Oman_samples_sum_all_Xanthomonadaceae %>% gather(key = "sample_ID", value = "rel_abund", -tax_short, -tax_long)

# pretty print
Oman_samples_sum_all_Xanthomonadaceae_gathered %>% select(-tax_long) %>% kable()
```

Add metadata map to gathered Xanthomonadaceae taxa data frame
```{r add-metadata-map-Xanthomonadaceae}
# join meta data map to Xanthomonadaceae taxa data frame
gathered_w_map <- left_join(Oman_samples_sum_all_Xanthomonadaceae_gathered, map_Oman_samples, by = "sample_ID")
```
...
```{r reorder-factor-levels-Xanthomonadaceae}
...
```{r fill-colors-Xanthomonadaceae}
...
fill_taxa_named_Xanthomonadaceae <- c(
  "genus Lysobacter" = "#56B4E9",
  "genus unassigned" = "#999999",
  "genus Pseudoxanthomonas" = "#D55E00",
  "genus Silanimonas" = "#000000")
```

set factor levels for wells so that they are ordered as desired on plot
```{r reorder-factor-levels-genus-Xanthomonadaceae-wells}
...
### Plot
```{r OM18-Xanthomonadaceae-bar-packers-all-filters, fig.width = 5, fig.height = 3.5, dpi=300}
# create plot
OM18_Xanthomonadaceae_bar <- gathered_w_map %>% ggplot(
  ...
  # axis styling
  scale_y_continuous(name = "16S rRNA gene read percentage", labels = formatter100)+
  scale_x_discrete(name = NULL)+
  
  # symbol styling
  scale_fill_manual(name = "taxon", values = fill_taxa_named_Xanthomonadaceae, labels = unname(latex2exp::TeX(c( "genus unassigned",
  "genus \\textit{Lysobacter}",
  "genus \\textit{Pseudoxanthomonas}",
  "genus \\textit{Silanimonas}")))) +
