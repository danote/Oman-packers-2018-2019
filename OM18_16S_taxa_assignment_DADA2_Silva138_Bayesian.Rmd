...
path <- "~/Desktop/Boulder/2018_fall/molecular_methods_ebio/Molecular-Methods/dada2/attempt_1_files/demux_extracted_OM18/data_for_packer_paper/gzipped_files" # CHANGE ME to the directory containing the fastq files after unzipping.
list.files(path)
```
...
write(asv_fasta, "ASVs_Silva_Bayes_packers.fa")

  # count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "ASVs_counts_Silva_Bayes_packers.txt", sep="\t", quote=F)

  # tax table:
asv_tax <- taxa_Silva
row.names(asv_tax) <- sub(">", "", asv_headers)
write.table(asv_tax, "ASVs_taxonomy_Silva_Bayes_packers.txt", sep="\t", quote=F)
```
...
write.table(tax_tab_for_mctoolsr_Silva, "data_output/tax_tab_DADA2_mctoolsr_OM18_Silva138_Bayes_packers.txt", row.names = FALSE, sep="\t", quote=F) # write a .txt file of the taxa table
```