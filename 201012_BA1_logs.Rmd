...
BA1_profiles_and_packer_geochem <- full_join((BA1_profiles_longer %>% mutate(sample_method = "Wireline logging")), (Oman_Geochem_BA1_longer_depth %>% mutate(sample_method = "Pumping with packer(s)")))
```
  ...
  mutate(
    tex_params = as_factor(parameter) %>%
      fct_recode(
        ...
        "Electrical conductivity$\\,$/$\\,$$\\lbrack$mS$\\cdot$cm$^{-1}$$\\rbrack$" = "cond_mS_per_cm"
  )
  )
```
...
BA1_profiles_plot <- BA1_profiles_and_packer_geochem %>% ggplot(aes(
  ...
  scale_size_manual(name = "Method", values = c("Wireline logging" = 0.2, "Pumping with packer(s)" = 2.5)) +
